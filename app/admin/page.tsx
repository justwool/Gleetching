...
import { FIGLET_FONTS } from '@/lib/figlet/fonts-manifest';
  ...
  const refresh = async () => {
    ...
    if (cols.ok) {
      ...
      if (settings.ok) {
        ...
        setFigletFont(settingPayload.figletFont || 'Standard');
  ...
  return (
    <section className="frame">
      ...
      {auth ? (
        <div style={{ margin: '8px 0', display: 'grid', gap: 6 }}>
          <div style={{ display: 'grid', gap: 4 }}>
            <label htmlFor="figletFont">FIGlet Wordmark Font (any name)</label>
            <input id="figletFont" list="figlet-font-list" value={figletFont} onChange={(e) => updateFont(e.target.value)} />
            <datalist id="figlet-font-list">
              {FIGLET_FONTS.map((font) => <option key={font} value={font} />)}
            </datalist>
          </div>
      ...
      <form onSubmit={submitCollection}>
        ...
        <input name="figletFont" placeholder="Standard" />
